...
class CurveSpeedController:
  ...
  def log_data(self, long_control_active, v_ego, sm):
    self.enable_training = v_ego > frogpilot_variables.CRUISING_SPEED
    ...
    if self.enable_training:
      ...
      if (self.training_timer >= frogpilot_variables.PLANNER_TIME and self.frogpilot_planner.driving_in_curve and not (sm["carState"].leftBlinker or sm["carState"].rightBlinker)):
    ...
    elif self.training_timer >= frogpilot_variables.PLANNER_TIME:
      progress = 0.0
      total_weight = 0.0
  ...
  def update_target(self, v_ego):
    ...
    if self.target_set:
      ...
      self.target = np.clip(self.target, frogpilot_variables.CRUISING_SPEED, csc_speed)
    else:
