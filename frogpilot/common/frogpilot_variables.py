...
def get_frogpilot_toggles(sm=None):
  toggles = sm["frogpilotPlan"].frogpilotToggles if sm is not None else None
  if toggles:
  ...
  if not hasattr(get_frogpilot_toggles, "_params_memory"):
    ...
    get_frogpilot_toggles._frogpilot_variables = FrogPilotVariables()
  elif get_frogpilot_toggles._params_memory.get_bool("FrogPilotTogglesUpdated"):
    get_frogpilot_toggles._frogpilot_variables.update()

  return get_frogpilot_toggles._frogpilot_variables.frogpilot_toggles
...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme="stock", started=False):
    ...
    toggle.cruise_increase = self.get_value("CruiseButtonIncrement", cast=float, condition=(quality_of_life_longitudinal and not pcm_cruise))
    toggle.cruise_increase_long = self.get_value("CruiseButtonIncrementLong", cast=float, condition=(quality_of_life_longitudinal and not pcm_cruise))
