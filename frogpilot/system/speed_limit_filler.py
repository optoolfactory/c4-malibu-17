...
class MapSpeedLogger:
  ...
  def wait_for_api(self):
    while not frogpilot_utilities.is_url_pingable(OVERPASS_STATUS_URL):
  ...
  def log_speed_limit(self):
    ...
    self.dataset_additions.append({
      ...
      "road_name": road_name,
      "road_width": frogpilot_utilities.calculate_lane_width(self.sm["modelV2"].laneLines[1], self.sm["modelV2"].laneLines[2]),
      "source": source,
