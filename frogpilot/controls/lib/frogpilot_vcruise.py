...
class FrogPilotVCruise:
  ...
  def update(self, long_control_active, now, time_validated, v_cruise, v_ego, sm, frogpilot_toggles):
    ...
    if (long_control_active and v_ego > frogpilot_variables.CRUISING_SPEED and self.frogpilot_planner.road_curvature_detected and frogpilot_toggles.curve_speed_controller):
    ...
    if force_stop_enabled and not self.override_force_stop:
      ...
      v_cruise = min((self.tracked_model_length // frogpilot_variables.PLANNER_TIME), v_cruise)
