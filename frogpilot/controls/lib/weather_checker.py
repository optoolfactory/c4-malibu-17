...
class WeatherChecker:
  def __init__(self, FrogPilotPlanner):
    ...
    self.api_token, self.build_metadata, self.device_type, self.dongle_id = frogpilot_utilities.get_frogpilot_api_info()
  ...
  def update_weather(self, now, frogpilot_toggles):
    if self.last_gps_position and self.last_updated:
      distance = frogpilot_utilities.calculate_distance_to_point(
    ...
    def make_request():
      ...
      for attempt in range(1, MAX_RETRIES + 1):
        try:
          response = self.session.post(f"{frogpilot_variables.FROGPILOT_API}/weather", json=payload, headers={"Content-Type": "application/json"}, timeout=10)
